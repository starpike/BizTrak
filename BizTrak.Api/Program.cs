...
builder.Services.AddDbContext<BizTrakDbContext>(options =>
{
    options.UseNpgsql(builder.Configuration.GetConnectionString("FinanceDb"), 
        x => x.MigrationsAssembly("BizTrak.Data"));
});
...
DbInitializer.Seed(app.Services.CreateScope().ServiceProvider.GetRequiredService<BizTrakDbContext>());
